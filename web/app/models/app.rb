class App < ActiveRecord::Base
  has_many :services, :conditions => { :serviceable_type => 2 }, :foreign_key => 'serviceable_id', :dependent => :destroy
  has_many :disco_services, :conditions => { :serviceable_type => 2 }, :foreign_key => 'serviceable_id', :dependent => :destroy
  ...
  def ctrl_service
    @ctrl_service || (@ctrl_service = Service.first(:conditions => {:serviceable_type => 2, :serviceable_id => self.id, :ctrl_state => 1}))
  end
