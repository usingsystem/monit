class Host < ActiveRecord::Base
  ...
  has_many :services, :conditions => { :serviceable_type => 1 }, :foreign_key => 'serviceable_id', :dependent => :destroy
  has_many :disco_services, :conditions => { :serviceable_type => 1 }, :foreign_key => 'serviceable_id', :dependent => :destroy
  belongs_to :agent 
  #has_one :control_service, :conditions => { :serviceable_type => 1, :ctrl_state => 1 }, :foreign_key => 'serviceable_id', :class_name => "Service"
  ...
  def before_create
    uuid = Host.find_by_sql("select uuid() uid from dual")[0].uid
    send("uuid=",uuid)
  end
  ...
  def ctrl_service
    @ctrl_service || (@ctrl_service = Service.first(:conditions => {:serviceable_type => 1, :serviceable_id => self.id, :ctrl_state => 1}))
  end

  def gen_ctrl
    ctrl_type = self.type.ctrl_service_types.first
    if ctrl_type
      ctrl = Service.gen(:type_id => ctrl_type.id, :tenant_id => self.tenant_id, :serviceable_id => self.id, :ctrl_state => 1, :agent_id => self.agent_id)  
      ctrl.save
    end
  end
