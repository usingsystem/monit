class Report
  class << self
      ...
      if data.size > 0
        ...
        data.each do |x|
          ...
          if s = services[uuid]
            ...
            d[:serviceable_type] = s.serviceable_name
    ...
    def find_resource uuids
      Service.all({ 
        :select => "services.id, services.uuid, services.name, services.serviceable_type, t1.id host_id, t1.name host_name, t2.id app_id, t2.name app_name",
        :joins=>"LEFT JOIN hosts t1 ON t1.id = services.serviceable_id AND services.serviceable_type = 1 LEFT JOIN apps t2 ON t2.id = services.serviceable_id AND services.serviceable_type = 2",
