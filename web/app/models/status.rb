class Status
  ...
  def history options
    ...
    data = self.class.get(config[:family], @uuid, :start => start.to_s, :finish => finish.to_s, :count => 1000)
  ...
  def handle data
    data.each do |key, val|
      ...
      data[key] = self.class.to_percent(val)
    end
    data
  end
  ...
  class << self
    ...
    def get family, uuid, options
      ...
      puts "#{family} Get (#{(Time.now - t1)*1000}ms) #{uuid.inspect}(#{options.inspect}) "
      data
    end
    ...
    def history objects, options
      ...
      data = get(config[:family], objects.collect{|s| s.uuid }, :start => options[:start].to_i.to_s, :finish => options[:finish].to_i.to_s, :count => 5000)
      data.each do |uuid, val|
        objects.select{|x| x.uuid == uuid}.each{|o|o.status_data.set_history(val, options)}
      end
    ...
    def to_percent data
      ...
      new = {}
      data.each do |k, v|
        new["_" + k] = total > 0 ? (v.to_f/total).round(3)*100 : 0 if k[0] != 95
      end
      new.each do |k, v|
        data[k] = v
      end
