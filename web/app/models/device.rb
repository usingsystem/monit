class Device < ActiveRecord::Base
  has_many :services, :conditions => { :serviceable_type => 4 }, :foreign_key => 'serviceable_id', :dependent => :destroy
  has_many :disco_services, :conditions => { :serviceable_type => 4 }, :foreign_key => 'serviceable_id', :dependent => :destroy
  belongs_to :agent 
  ...
  def ctrl_service
    @ctrl_service || (@ctrl_service = Service.first(:conditions => {:serviceable_type => 4, :serviceable_id => self.id, :ctrl_state => 1}))
  end
