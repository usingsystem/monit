...
module ApplicationHelper
  ...
  def type_link_to text, url
   ...
   def group_link_to text, url
    ...
    unless text.blank?
      ...
      key = ""
      if keys.size==0
       str << link_to(text, url.update(:group => text))
      end
      while keys.size > 0
        k = keys.shift
        key << "/" + k
        str << "/" + link_to(k, url.update(:group => key))
      end
    end
    str
  end
  ...
  def board_header content_or_options_with_block = nil,options =nil,&block
    content_tag_with_default_class("h4","board-header clearfix",content_or_options_with_block,options,&block)
  end
