namespace :notification do
  ...
  task :mail_alerts => :environment do
    ...
    @alert_notifications = AlertNotification.all({
      :include => ['service', 'user', 'tenant'] + AlertNotification.source_names,
      :conditions => {:method => 0, :status => 0}
    })
    if @alert_notifications.size > 0
      notifications = []
      @alert_notifications.each do |noti|
        tmail = Notifier.deliver_alert(noti)
        notifications << {
          :method => 0,
          :status => 1,
          :user_id => noti.user_id,
          :address => noti.user.email,
          :contact => noti.user.login,
  ...
  task :mail_report, [:frequency] => :environment do |t, args|
    ...
    @users = User.all(:conditions => {frequency.to_sym => true}, :include => [:apps, :sites, :hosts, :devices, :tenant]).select{|u| u.apps.size > 0 or u.sites.size > 0 or u.hosts.size > 0 or u.devices.size > 0 }
    notifications = []
    @users.each do |user|
