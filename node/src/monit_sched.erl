...
update_task(TaskId, TaskAttrs) ->
    gen_server:cast(?MODULE, {update_task, TaskId, TaskAttrs}).
...
%%--------------------------------------------------------------------
handle_cast({update_task, TaskId, TaskAttrs}, State) ->
    case ets:lookup(monit_task, TaskId) of
    [Task] ->
...
handle_cast({reschedule, TaskId, Interval}, State) ->
    ?INFO("reschedule ~p", [TaskId]),
    case ets:lookup(monit_task, TaskId) of
    [Task] ->
        %?INFO("reschedule task: ~p ", [Task]),
...
handle_info({run, TaskId}, State) ->
    case ets:lookup(monit_task, TaskId) of
    [Task] -> 
        spawn(fun() -> monit_task:run(Task) end);
    [] ->
