...
reschedule(SchedTime, #monit_task{taskid= Uuid, 
    ...
    smarta_sched:update_task(Uuid, [{status, Status}, 
            {status_type, StatusType},
            {current_attempts, CurrentAttempts},
            {duration, Duration},
            {latency, Latency},
...
emit_status(Status, Summary, DataItems, #monit_task{taskid = Uuid, 
    ...
    StatusData = [{uuid, Uuid}, 
        ...
        {last_check_at, LastCheckAt},
        {current_attempts, CurrentAttempts}],
    smarta:send({status, StatusData, DataItems}).
