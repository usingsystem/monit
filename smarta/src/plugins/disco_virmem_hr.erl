%%%----------------------------------------------------------------------
%%% File    : disco_virmem_hr.erl
%%% Author  : Ery Lee <ery.lee@gmail.com>
%%% Purpose : disco window virtual memory by host-resources-mib
%%% Created : 29 Mar 2010
%%% License : http://www.monit.cn
%%%
%%% Copyright (C) 2007-2010, www.monit.cn
%%%----------------------------------------------------------------------
-module(disco_virmem_hr).
...
run(Args) ->
    ...
    {ok, Storages} ->
        ...
        Summary = "Discover virtual memory success!",
        Services = 
        lists:map(fun(Mem) -> 
            ...
            {service, check_virmem_hr, "虚拟内存", Params, "虚拟内存使用率 = " ++ integer_to_list(Usage) ++ "%"}
        end, Memories),
        {ok, Summary, Services};
    {error, _Reason} ->
