%%%----------------------------------------------------------------------
%%% File    : disco_sw_if.erl
%%% Author  : Ery Lee <ery.lee@gmail.com>
%%% Purpose : disco switch router interfaces
%%% Created : 28 Mar 2010
...
run(Args) ->
    ...
    case sesnmp:get_table(Host, Port, [?ifIndex,?ifDescr,?ifType,?ifSpeed,?ifOperStatus,?ifAdminStatus], AgentData) of
    {ok, IfEntries} ->
        IfEntries1 = 
        ...
        lists:map(fun(IfEntry) -> 
            ...
            Params = "host=${device.addr}&port=${device.port}&community=${device.community}&ifindex=" ++ integer_to_list(IfIndex),
