...
class Tenant < ActiveRecord::Base
  ...
  def settle
    ...
    range = "[#{I18n.l(Date.today, :format => :long)} - #{I18n.l(next_paid_at, :format => :long)}]"
    now = Date.today
    update_attributes({
    ...
    Bill.create(:operator_id => operator_id, :balance => balance, :tenant_id => id, :amount => last, :summary => "取消套餐#{package.title}#{range}") if last > 0
  end
  ...
  def use_package(package_id, month_num)
    ...
    Bill.create(:operator_id => operator_id, :balance => balance, :tenant_id => id, :amount => -cost, :summary => "使用套餐#{_package.title}#{range}") if cost > 0
  end
  ...
  def pay_package
    if month_num > current_month
      ...
      range = "[#{I18n.l(current_paid_at, :format => :long)} - #{I18n.l(next_paid_at, :format => :long)}]"
      Bill.create(:operator_id => operator_id, :balance => balance, :tenant_id => id, :amount => -cost, :summary => "使用套餐#{package.title}#{range}") if cost > 0
    else
      #套餐结束，重置
      update_attributes({
        :package_id => nil,
