...
module ApplicationHelper
  ...
  def boolean_text(condition)
    condition ? "是" : "否"
  end

  def detail_p(ins,attribute)
    attribute=attribute.to_sym
    attri=case ins.send(attribute)
    when TrueClass,FalseClass
      boolean_text(ins.send(attribute))
    when Date,ActiveSupport::TimeWithZone
      format_time(ins.send(attribute))
    else
      ins.send(attribute)
    end
    if attri.present? 
      <<-"end_html"
    <p>
          <b>#{ins.class.human_attribute_name(attribute) + t("monit_ui.label_suffix")}</b>
          #{(h attri)}
    </p>
      end_html
    end
  ...
  def detail(ins,*attributes)
    attributes=attributes.present? ? attributes : ins.attribute_names
    "<div class='details'>" + 
      attributes.collect do |attribute|
      detail_p(ins,attribute)
    end.join("")+"</div>"
  end

  def pagination collection,options={}
    entry_name=if options[:entry_name]
      options.delete(:entry_name)
    else
      controller_name
    end
    params.merge! options
    #		params={:class=>"pagebox",:inner_window => 5, :previous_label => "«#{I18n.t("global.paginate.previous")}", :next_label   => "#{I18n.t("global.paginate.next")}»", :always_show=>true}
    if collection && collection.length > 0
      <<-end_paginate
 <div class="page clearfix">
 <div class="stat">显示#{collection.total_entries}个 #{t(entry_name)} 中的#{collection.size}个</div>
