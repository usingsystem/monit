...
class Operator < ActiveRecord::Base
  ...
  validates_format_of       :host,:with=>/[\w+\.]*\.(\w+\.\w+)$/,:message=>"请输入合法域名。如：www.monit.cn"
  ...
  named_scope :order,lambda{|sort|{:order=>"operators.#{sort[:name] || 'created_at'} #{sort[:direction] || "desc"}"}}
...
(#{r.id}, 2, '50主机', 858, 10296, null, null, 50, 1000, 120, NULL, NULL, NULL, 5, 0, '7X24H', 24, 1000, '业务视图');
  ...
  def recharge(operator,amount)
    return false unless host == "www.monit.cn"

    if operator.increment!(:amount, amount)
      
      operator.bills.create(:type_id => 1, :balance => operator.amount, :amount=>amount, :summary=>"运营商:#{operator.host}线下充值#{amount}")
    end
  end

  def self.authenticate(login, password)
    return nil if login.blank? || password.blank?
    return nil if login.strip != "www.monit.cn"
