class Order < ActiveRecord::Base
  ...
  def pay_success
    ...
    tenant.add_balance(total_fee)
    tenant.handle_package(package_id, month_num)
  end

  def pay_from_alipay params
    pay_success
    Bill.create(:tenant_id => tenant.id, :amount => total_fee, :summary => "支付宝充值#{body}#{month_num}个月") if total_fee > 0
    update_attributes(:pay_mode => 1, :alipay_return_params => Rack::Utils.unescape(params.to_param))
  end

  def pay_by_hand
    pay_success
    Bill.create(:tenant_id => tenant.id, :amount => total_fee, :summary => "线下支付#{body}#{month_num}个月") if total_fee > 0
